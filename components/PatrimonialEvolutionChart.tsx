...
const PatrimonialEvolutionChart: React.ForwardRefRenderFunction<HTMLDivElement, PatrimonialEvolutionChartProps> = ({ portfolioData }, ref) => {
    ...
    const chartDataWithBenchmarks = useMemo(() => {
        ...
        if (activeBenchmarks.length === 0) return baseData.map(d => ({ ...d, 'Carteira': d.value }));

        const dataWithBenchmarks = baseData.map(point => ({ ...point, 'Carteira': point.value }));

        activeBenchmarks.forEach(benchmark => {
            const benchmarkData = generateBenchmarkData(benchmark as any, baseData);
            benchmarkData.forEach((bPoint, index) => {
                if(dataWithBenchmarks[index]) {
                    dataWithBenchmarks[index][benchmark] = bPoint.value;
                }
            });
        });
        return dataWithBenchmarks;
    }, [portfolioData, period, activeBenchmarks]);
            ...
            <div className="h-80 -ml-4">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartDataWithBenchmarks} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                        ...
                        <Area type="monotone" dataKey="Carteira" stroke="#3B82F6" strokeWidth={2} fillOpacity={1} fill="url(#colorCarteira)" name="Carteira" />
                        ...
                        <Brush dataKey="date" height={30} stroke="#3B82F6" fill="rgba(128, 128, 128, 0.1)" tickFormatter={(dateStr) => {
                                const date = new Date(dateStr + 'T00:00:00');
                                return date.toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' });
                            }}>
                            <AreaChart data={chartDataWithBenchmarks}>
                                <Area dataKey="Carteira" fill="#3B82F6" stroke="#3B82F6" fillOpacity={0.5} />
                            </AreaChart>
                        </Brush>
                    </AreaChart>
                </ResponsiveContainer>
